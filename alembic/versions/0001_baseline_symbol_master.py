...
Revision ID: 0001_baseline_symbol_master
Revises:
...
def upgrade() -> None:
    ...
    op.execute("CREATE INDEX assets_asset_type_idx ON symbol_master.assets (asset_type)")
    op.execute("CREATE INDEX assets_active_idx ON symbol_master.assets (active)")
    op.execute(
    ...
    op.execute("CREATE INDEX vendor_symbols_asset_id_idx ON symbol_master.vendor_symbols (asset_id)")
    op.execute("CREATE INDEX vendor_symbols_active_idx ON symbol_master.vendor_symbols (active)")
    op.execute(
        """
    ...
    op.execute("CREATE INDEX ingestion_runs_status_idx ON symbol_master.ingestion_runs (status)")


def downgrade() -> None:
    op.execute("DROP TABLE IF EXISTS symbol_master.ingestion_runs")
    op.execute("DROP TABLE IF EXISTS symbol_master.vendor_symbols")
    op.execute("DROP TABLE IF EXISTS symbol_master.etf_profiles")
    op.execute("DROP TABLE IF EXISTS symbol_master.assets")
    op.execute("DROP TABLE IF EXISTS symbol_master.exchanges")
    op.execute("DROP TABLE IF EXISTS symbol_master.vendors")
