...
### What changed
- Corrected the Day 3 Massive/Polygon smoke documentation to use `quant-symbols-massive`.
- Documented the separation between `python3 -m quant_symbols.cli` database/symbol commands and the Massive smoke console script.
...
- Existing CLI boundaries are now documented as implemented: project DB/symbol commands use `python3 -m quant_symbols.cli`, and the retrieval-only Massive smoke check uses `quant-symbols-massive`.

### Massive / Polygon integration impact
- No Massive/Polygon client code changed.
- Documentation no longer claims the Massive smoke CLI supports unimplemented `vendors massive tickers`, `--fixture`, `--dry-run`, `--market`, or `--active` options.
...
- Day 3 docs, README usage, and Quant Software Developer notes now agree that the Massive client smoke command is `quant-symbols-massive`.
...
- Passed: `python3 -m quant_symbols.cli symbols sync --fixture tests/fixtures/massive --dry-run`; output was `symbols_sync=ok vendor=massive mode=fixture pages=1 records_seen=5 raw_payloads=0 symbols_inserted=5 symbols_updated=0 symbols_unchanged=0 deactivated=0 reactivated=0 exchanges_inserted=3 vendor_ids_inserted=5 aliases_inserted=10 skipped=0 warnings=0 errors=0`.
...
- Could not validate installed `quant-symbols-massive` console script because this runner has Python 3.8.10, no `python3.12`, and the project declares `requires-python = ">=3.12"`. The command is not on PATH without package installation.
